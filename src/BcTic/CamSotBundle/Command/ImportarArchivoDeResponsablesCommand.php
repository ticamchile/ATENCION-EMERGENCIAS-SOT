...
class ImportarArchivoDeResponsablesCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('ssee-sot:importar-archivo-de-responsables')
            ->setDescription('IMPORTAR ARCHIVO DE RESPONSABLES SOT');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln("COMENZANDO PROCESO DE RESPONSABLES");

        $entities = $this->em->getRepository('BcTicCamSotBundle:ArchivoDeEvento')->findBy(
          array('status' => 'PENDING', 'tipo' => 'RESPONSABLES'),
          array(),
          1,
          0
          );
    ...
    if (($handle = fopen($file, "r")) == false) throw new \Exception("ARCHIVO NO SE PUEDE PROCESAR."); 
     ...
     while (($data = fgetcsv($handle, 0, $archivoDeEvento->getColumnsSeparator())) !== false) {
        ...
        if (count($data) != 9) { 
          ...
          $msg = "ARCHIVO LINEA ".$row." NO TIENE 9 COLUMNAS, TIENE ".count($data).". NO SE PUEDE PROCESAR EL ARCHIVO Y SE MARCA COMO NULO.";
       ...
       try {
         if ($row > 0) {
           ...
           $hits++;
           if ($hits == 50000) {
             $output->writeln("      > ".$hits." RESPONSABLES PERSISTIDOS ");
             return;
           }
         } else {
     ...
     $output->writeln("      > ".($row - 1)." RESPONSABLES PERSISTIDOS ");
  ...
  private function rowToObject($data =  array(),InputInterface $input, OutputInterface $output) {
    ...
    if (is_object($evento)) {
      //BORRO LAS CAUSAS DE ESTE EVENTO
      foreach ($evento->getCausasDeEvento() as $causaDeEvento) {
        $this->em->remove($causaDeEvento);
        $this->em->flush();
      }
    } else {
    ...
    foreach ($data as $nemo => $value) {
      ...
      if ($nemo == 1) continue; //1 NO APLICA
      if ($nemo == 3) continue; //3 es la descripción del  elemento responsable
      if ($nemo == 5) continue; //3 es la descripción del  ambito
      if ($nemo == 7) continue; //3 es la descripción de la condición
      if ($nemo == 8) continue; //8 OBSERVACIONES
   
      //Si está vacía la propiedad, no importa por que debe hacer el "espacio para la edición".
      $propiedad = new CausaDeEvento();
